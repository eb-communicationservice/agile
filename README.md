 ...
 * リーダーさんも軌道修正しやすそうだった。
 * 内容がわからないタスクは見積もらないので仕様変更に振り回されることがなかった。
   * 担当者(企画側)から詳細が出てくるまで着手しないことをネゴっていたため可能。
 ...
 * プロジェクトの最終決定が出来ないため、スコープの調整がほぼ不可となり、最終的には全ての機能を実装する必要あり。全ての機能を実装するならウォーターフォールと同じ。
